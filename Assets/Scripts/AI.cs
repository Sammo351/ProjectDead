...
public class AI : MonoBehaviour, IDetectSound, IDetectSight
{
    ...
    public void OnDetectSound(Vector3 position, float p)
    {

        if (IsTargetInSoundRange(position))
        {
            SetTarget(position, p);
        }
    }
    ...
    public void SetTarget(Transform t, float priority)
    {
    ...
    public void SetTarget(Vector3 t, float priority)
    {
        if (priority >= targetPriority)
        {
            targetPriority = priority;
