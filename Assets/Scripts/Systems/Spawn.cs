...
public class Spawn
{
    ...
    public static void SurroundWithPriority(Vector3 position, int count = 1, float priority = 0, float radius = 35f, bool attack = false)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (attack)
            {
                list[rand].SpawnWithPriority(1, priority, position);
            }
            else
            {
                list[rand].SpawnWithPriority(1, priority);
            }
        }
    }
